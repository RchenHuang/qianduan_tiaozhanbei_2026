 ...
 * @param {import('vue').Ref<string>} mode - 游戏模式（'number' | 'color' | 'reverse'）
 * @returns {Object} 游戏状态、方法和性能指标
 */
export function useSchulteGame(gridSize, mode) {
  // 组合反应时间追踪
  ...
  function startGame() {
    ...
    // 降序模式从最大数字开始，普通模式从1开始
    currentTarget.value = mode.value === 'reverse' ? totalNumbers.value : 1
    errorCount.value = 0
    reaction.reset()
  }
