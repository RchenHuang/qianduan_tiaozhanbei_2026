var Story = (function() {
  ...
  // Advances to the next story item
  var advance = function() {
    console.log('advance() called, current bookmark:', bookmark, 'story length:', story.length);
    
    if(bookmark < story.length - 1) {
      // Move to the next item
      bookmark++;
      
      console.log('Next bookmark:', bookmark, 'type:', story[bookmark].type);
      
      // If it's an instruction that was already visited, skip it
      while(story[bookmark].type == 'instruction' && visited(bookmark)) {
        console.log('Skipping visited instruction at bookmark:', bookmark);
        bookmark++;
      ...
      console.log('Setting bookmark to:', bookmark);
      setBookmark(bookmark);
    } else {
      console.log('Already at end of story');
    }
  };
