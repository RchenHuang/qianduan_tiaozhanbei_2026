...
$(document).ready(function() {
  console.log('DOM is ready, starting game initialization...');
  mediator.publish('loader_dom_ready'); // Notify everyone we can work now
  console.log('Requesting story.json...');
...
var Board = (function() {
    ...
    if(isWin()) {
      if (board && board.length > 0) {
        board.fadeOut(options.fade, function() {
          var score = calculateParityScore(level);
  ...
  function cell(x, y, value) {
    if (cells.length === 0 || !cells[x] || !cells[x][y]) return 0;
  ...
  var render = function(data) {
    ...
    // Set the level text
    if (levelLink && levelLink.length > 0) {
  ...
  // Calculate parity game score
  var calculateParityScore = function(levelData) {
    var levelNum = parseInt(levelData.number);
    var mode = levelData.mode;
    
    var baseScore = 60 + (levelNum * 5);
    ...
    var moveBonus = Math.max(0, 10 - moves);
    
    var totalScore = baseScore + modeBonus + moveBonus;
  ...
  var calculateMoves = function() {
    ...
    for(var i = 0; i < level.contents.length; ++i) {
      var x = i % 3;
      var y = Math.floor(i / 3);
      var currentVal = parseInt(cells[x][y].html());
      var initialVal = level.contents[i];
...
var Selector = (function() {
  ...
  var snapTo = function(x, y) {
    ...
    if (selector && selector.length > 0) {
      ...
      var relativeTop = cellOffset.top - boxOffset.top;
      var relativeLeft = cellOffset.left - boxOffset.left;
      selector.animate({
        top: (relativeTop + cellHeight / 2) + 'px',
        left: (relativeLeft + cellWidth / 2) + 'px'
      }, 100, function() {
        //todo when complete
      });
    }
  ...
  var resnap = function() {
    if (selector && selector.length > 0) {
      ...
      var relativeTop = cellOffset.top - boxOffset.top;
      var relativeLeft = cellOffset.left - boxOffset.left;
      selector.css('top', (relativeTop + cellHeight / 2) + 'px');
      selector.css('left', (relativeLeft + cellWidth / 2) + 'px');
    }
  };
...
var Story = (function() {
  ...
  var getStory = function() {
    console.log('Using embedded story data');
    if (typeof embeddedStoryData !== 'undefined') {
      story = embeddedStoryData;
      console.log('Story data loaded successfully, total items:', story.length);
      mediator.publish('story_num_levels', getNumLevels());
      mediator.publish('story_story_loaded');
    } else {
      console.error('embeddedStoryData is not defined. Make sure story-data.js is loaded before production.js');
      console.log('Attempting to load story.json as fallback...');
      $.getJSON('story.json', function(data) {
        console.log('story.json loaded successfully:', data);
        story = data;
        mediator.publish('story_num_levels', getNumLevels());
        mediator.publish('story_story_loaded');
      }).fail(function(jqXHR, textStatus, errorThrown) {
        console.error('Failed to load story.json:', textStatus, errorThrown);
        console.error('Response:', jqXHR.responseText);
        console.error('Status:', jqXHR.status);
        console.error('URL:', 'story.json');
      });
    }
  };
